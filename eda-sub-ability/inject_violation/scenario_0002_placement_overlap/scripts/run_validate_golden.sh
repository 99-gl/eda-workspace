...
# Fresh-loads the formal injected checkpoint and saves the validated repair.

set -euo pipefail
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
scenario_dir="$(cd "${script_dir}/.." && pwd)"
mkdir -p "${scenario_dir}/logs"
innovus -nowin -files "${scenario_dir}/validate_golden.tcl" > "${scenario_dir}/logs/validate_golden.log" 2>&1
grep -q 'PLACEMENT_OVERLAP_GOLDEN_VALIDATION_COMPLETE' "${scenario_dir}/logs/validate_golden.log"
...
echo PLACEMENT_OVERLAP_GOLDEN_RUNNER_COMPLETE
