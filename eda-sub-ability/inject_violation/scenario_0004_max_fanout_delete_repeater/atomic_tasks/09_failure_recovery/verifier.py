...
def main():
    ...
    required = ["bad_fanout.rpt", "fanout.rpt", "transition.rpt", "capacitance.rpt", "setup.rpt", "hold.rpt", "placement.rpt", "connectivity.rpt", "drc.rpt"]
    if proc.returncode != 0 or "MAX_FANOUT_ATOMIC_09_COMPLETE" not in text or any(not os.path.isfile(os.path.join(reports, name)) or os.path.getsize(os.path.join(reports, name)) == 0 for name in required):
        print("FAIL")
        return 1
    ...
    bad, fanout = read_report("bad_fanout.rpt"), read_report("fanout.rpt")
    ...
    ok = "1 violation is real" in bad and "0 violation is real" in fanout and sm is not None and hm is not None and float(sm.group(1)) >= 0 and float(hm.group(1)) >= 0
